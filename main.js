...
function generateCard(idea) {
  ...
  bottomContainer.insertAdjacentHTML(
    "afterbegin",
    `<article class="bottom__article--card"data-id="${idea.id}">
    <header class="bottom__header--card">
      ...
      <section class="bottom__section--card">
        <h3 class="bottom__title--card" contenteditable="true">${
          idea.title
        }</h3>
        <p class="bottom__paragraph--card" contenteditable="true">${
          idea.body
        }</p>
        </section>
        <footer class="bottom__footer--card">
          <img class="bottom__icon--card bottom__img--upvote" src="images/upvote.svg" class="bottom__img" id="bottom__img--upvote" alt="upvote__button--inactive"><span class="bottom__span--card">Quality: ${
            idea.qualityArray[idea.quality]
          }</span><img class="bottom__icon--card bottom__img--downvote" src="images/downvote.svg" id="bottom__img--downvote" alt="downvote__button--inactive">
        </footer>
      </article>`
  );
}
...
function instantiateIdea() {
  ...
  reloadConditions(idea);
}

function reloadConditions(idea) {
  generateCard(idea);
  clearDisplayMessage();
...
function starCard(e) {
  ...
  if (e.target.classList.contains("bottom__btn--star")) {
    ...
    var title = e.target
      .closest("article")
      .querySelector(".bottom__title--card").textContent;
    var body = e.target
      .closest("article")
      .querySelector(".bottom__paragraph--card").textContent;
...
function setText(e) {
  var locatedIndex = locateIndex(e);
  if (
    e.target.classList.contains("bottom__title--card") ||
...
function updateQualityCard(e, locatedIndex) {
  var qualityText = e.target
    .closest("article")
    .querySelector(".bottom__span--card");
...
function searchIdeas() {
  ...
  searchArray = searchArray.filter(function(idea) {
    return (
      ...
      idea.body.toLowerCase().includes(search)
    );
  });
...
function displayStarMessage() {
  bottomContainer.insertAdjacentHTML(
    "afterbegin",
    `<label class="bottom__display--message" id="bottom__display--message"type="">Star some ideas to see them displayed here!</label> `
  );
}
